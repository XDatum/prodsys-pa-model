...
${P2PA_MODEL_DIR}/bin/run.sh collector --verbose --force --work-dir ${NEW_TASKS_WORK_DIR} --config ${COLLECTOR_QUERY_CONFIG} --days 1 --output-type input >> ${LOG_FILE} 2>&1
${P2PA_MODEL_DIR}/bin/run.sh predictor --verbose --force --work-dir ${MODEL_WORK_DIR} --data-dir ${NEW_TASKS_WORK_DIR} --config ${PREDICTOR_QUERY_CONFIG} >> ${LOG_FILE} 2>&1
